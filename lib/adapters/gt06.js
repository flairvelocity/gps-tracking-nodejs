...
var adapter = function (device) {
  ...
  this.authorize = function (msg_parts) {
    ...
    // var buff = new Buffer.from('7878' + str + crcResult + '0d0a', 'hex');
    // var buff = new Buffer.from('787805010001d9dc0d0a', 'hex');
    //发送原始数据

    var str = msg_parts.start + msg_parts.length
      + parseInt(msg_parts.protocal_id, 16) + parseInt(msg_parts.serial_number, 16)
      + parseInt(msg_parts.error_check, 16) + parseInt(msg_parts.finish, 16);

    // var str = msg_parts.start+msg_parts.length
    //           +msg_parts.protocal_id+msg_parts.serial_number
    //           +msg_parts.error_check+msg_parts.finish;

    var buff = new Buffer.from(str, 'hex');
