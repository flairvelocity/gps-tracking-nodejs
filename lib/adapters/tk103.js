...
var adapter = function (device) {
	...
	*******************************************/
	...
	this.get_ping_data = function (msg_parts) {
		...
		var protoData = {
			date: str.substr(0, 6),
			valid: str.substr(6, 1),
			latitude: str.substr(7, 9),
			lat_indicator: str.substr(16, 1),
			longitude: str.substr(17, 10),
			long_indicator: str.substr(27, 1),
			speed: str.substr(28, 5),
			time: str.substr(33, 6),
			orientation: str.substr(39, 6),
			io_state: str.substr(45, 8),
			mile_post: str.substr(53, 1),
			mile_data: str.substr(54, 8)
		};
		// format
		var io_state = f.splitIoState(protoData.io_state)
		var res = {
			time: f.strToDateTime(protoData.date, protoData.time),
			valid: protoData.valid,
			latitude: f.minute_to_decimal(protoData.latitude, protoData.lat_indicator),
			longitude: f.minute_to_decimal(protoData.longitude, protoData.long_indicator),
			speed: parseFloat(protoData.speed),
			orientation: parseFloat(protoData.orientation),
			mile_post: protoData.mile_post,
			mile: f.hex_to_int(protoData.mile_data),
			raw: str
		};

		res = Object.assign(res, io_state);
